...
@section('content')
                    ...
                    <div class="w-full flex justify-center sm:mt-20">
                        <div class="bg-dark rounded-lg p-6">
                            ...
                            <div onclick="kelik('stats-1', 'forms-1')">
                                <div class="font-bold text-white text-3xl bg-{{ $latestUsrAbsen->status }} hover:bg-opacity-80 inline-block px-12 sm:px-14 py-4 rounded-lg cursor-pointer">{{ strtoupper($latestUsrAbsen->desc) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:flex hidden lg:h-[125%] items-center">
                    <div class="w-full flex flex-col items-center text-center sticky top-20">
                        ...
                        <div class="flex justify-between items-center w-[55%] xl:w-[45%] bg-dark rounded-lg px-10 py-8">
                            ...
                            <div onclick="kelik('stats-1', 'forms-1')">
                                <div class="font-bold text-white text-3xl bg-{{ $latestUsrAbsen->status }} hover:bg-opacity-80 inline-block px-10 py-6 rounded-lg cursor-pointer">{{ strtoupper($latestUsrAbsen->desc) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="block lg:flex flex-col justify-between h-full">
                    ...
                    @foreach ($userAbsens as $userAbsen)
                            ...
                            <div id="stats-{{ $i }}" class="bg-white border rounded-lg border-gray p-4 flex justify-between items-center mb-4 overflow-hidden">
                                ...
                                <a class="w-auto">
                                    <div onclick="kelik('stats-{{ $i }}', 'forms-{{ $i }}')" class="bg-{{ $userAbsen->status }} hover:bg-opacity-80 rounded-lg py-2 px-4 w-full flex justify-between items-center cursor-pointer">
                                        <div class="pr-2">
                                            <h2 class="font-semibold text-white text-2xl leading-5">{{ $userAbsen->desc }}</h2>
                        ...
                        <div id="forms-{{ $i }}" class="bg-white border rounded-lg border-gray p-4 mb-4 hidden">
                            ...
                            <form action="/member/absen/add/{{ $userAbsen->id }}" method="post">
                                @csrf
                                @if($userAbsen->desc != 'Alpha')
                                    <div class="">
                                        <h2 class="font-semibold text-xl">Sistem Kerja</h2>
                                        <div class="flex justify-between pr-12">
                                            @foreach($works as $work) 
                                                <label for="{{ $work }}-{{ $i }}">
                                                    <input {{ ($userAbsen->work == $work) ? 'checked' : '' }} {{ ($userAbsen->work != '') ? 'disabled' : '' }} required value="{{ $work }}"
                                    ...
                                    <div class="my-2">
                                        ...
                                        <label for="desc-tugas-{{ $i }}">
                                            <select onchange="kelikk('desc-tugas-{{ $i }}', 'other-desc-{{ $i }}');" {{ ($userAbsen->task != '') ? 'disabled' : '' }} name="tugas" id="desc-tugas-{{ $i }}" class="p-1 w-full rounded-lg my-2" required>
                                                <option value="Bot" {{ $userAbsen->task == 'Bot' ? 'selected' : '' }}>Bot</option>
                                                <option value="Clone" {{ $userAbsen->task == 'Clone' ? 'selected' : '' }}>Clone</option>
                                                <option {{ ($userAbsen->task != 'Bot' && $userAbsen->task != 'Clone') ? 'selected' : '' }}>Lainnya</option>
                                            </select>
                                        </label>
                                        <label class="{{ ($userAbsen->task == 'Bot' || $userAbsen->task == 'Clone') ? 'hidden' : 'block' }}">
                                            <input type="text" id="other-desc-{{ $i }}" name="tugas-lainnya" placeholder="deskripsi tugas" class="p-1 w-full rounded-lg" required>
                                        </label>
                                    </div>
                                @else
                                    <div class="my-2">
                ...
                <div class="fixed lg:right-24 xl:right-16 2xl:right-14 top-20 hidden lg:flex flex-col w-1/2 lg:w-[400px] 2xl:w-[500px]">
                    <div class="flex justify-between items-center bg-dark rounded-lg py-4 px-7 sticky top-20">
                        ...
                        <div  onclick="kelik('stats-1', 'forms-1')">
                            <div class="font-bold text-white text-2xl bg-{{ $latestUsrAbsen->status }} hover:bg-opacity-80 inline-block px-8 py-3 rounded-md cursor-pointer">{{ strtoupper($latestUsrAbsen->desc) }}</div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function kelikk(kelas, kelas2) {
            var optionValue = document.getElementById(kelas).value;
            
            if(optionValue != 'Bot' && optionValue != 'Clone') {
                var group1 = document.getElementById(kelas2);
                
                group1.classList.add('block');
                group1.classList.remove('hidden');
                
                var reqAttr = document.createAttribute('required');
                group1.setAttributeNode(reqAttr);
            } else {
                var group1 = document.getElementById(kelas2);
                ...
                group1.removeAttribute('required');
            }
        }
    </script>
@endsection